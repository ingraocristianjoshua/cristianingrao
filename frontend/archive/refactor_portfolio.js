...
// 1. Update CSS for grid
const oldCss = `        /* Windows Grid */
        .windows-container {
            display: grid;
        ...
        #win-bio { grid-column: 1 / 2; grid-row: 1 / 2; }
        #win-photo { grid-column: 2 / 3; grid-row: 1 / 2; }
        .mac-dock-container { grid-column: 1 / -1; grid-row: 2 / 3; }
        #win-bricklink { grid-column: 1 / -1; grid-row: 3 / 4; }
        #win-graduam { grid-column: 1 / -1; grid-row: 4 / 5; }
        #win-edu { grid-column: 1 / -1; grid-row: 5 / 6; }
        #win-contacts { grid-column: 1 / -1; grid-row: 6 / 7; }`;
...
// Update mobile queries
html = html.replace('.windows-container { grid-template-columns: 1fr; }', '.windows-container { display: flex; flex-direction: column; } \n            .col-12, .col-8, .col-7, .col-6, .col-5, .col-4 { width: 100%; grid-column: span 12; }');


// 2. Rewrite the HTML block for .windows-container
const windowsContainerRegex = /<div class="windows-container">([\s\S]*?)<\/div>\s*<\/div>\s*<\/div>\s*<script>/;
// Wait, the regex might be tricky. Let's just find the start and end of .windows-container manually
...
if (startIndex !== -1 && endIndex !== -1) {
    ...
    html = html.substring(0, startIndex) + newWindowsHtml + html.substring(endIndex);
    fs.writeFileSync('portfolio/index.html', html);
    console.log('Successfully refactored layout to multi-window clusters');
} else {
    console.log('Error: Could not find windows-container section.');
}
