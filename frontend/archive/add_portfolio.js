...
// 1. Replace the win-folder-projects window with win-portfolio
const oldWindowRegex = /<div class="mac-window" id="win-folder-projects"[\s\S]*?<!-- FOLDER WINDOWS \(FINDER\) -->\s*<div class="mac-window" id="win-folder-miniature"/g;
...
// 2. Update desktop icon to trigger win-portfolio instead of win-folder-projects
html = html.replace(/openWindow\('win-folder-projects'\)/g, "openWindow('win-portfolio')");

// Also, let's change the name of the desktop folder from "Projects" to "Portfolio"
// and the icon to a nice star folder or just "Portfolio" text.
html = html.replace(/<span class="folder-name">Projects<\/span>/g, '<span class="folder-name">Portfolio</span>');

fs.writeFileSync('index.html', html, 'utf8');
console.log("Updated HTML with win-portfolio");
